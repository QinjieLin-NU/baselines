...
class env(bew.base_env):
    ...
    def _get_observation(self):
        x, x_dot, theta, theta_dot = self._env.env.state
    ...
    def _build_env(self):
        ...
        _env_name = {
            'gym_swingup': 'BaselineSwingUp-v0'
        }
    ...
    def _set_dynamics_api(self):
        ...
        def set_state(data_dict):
            ...
            x, x_dot, theta, theta_dot = data_dict['start_state'][0],data_dict['start_state'][1],data_dict['start_state'][2],data_dict['start_state'][3]
            state = np.asarray([x, x_dot, theta, theta_dot])
            # reset the state
    ...
    def _set_reward_api(self):
        ...
        def reward(data_dict):
            ...
            l = self._env.env.l
            nextState = self._env.env.np_dynamics(state,action)
            x, theta, x_dot, theta_dot = nextState[0],nextState[1],nextState[2],nextState[3]
            j_target = np.array([0.0, 0.0, -1.0])
            delta_j1 = x - j_target[0]
            delta_j2 =  np.sin(theta) - j_target[1]
            delta_j3 =  np.cos(theta) - j_target[2]
            reward = delta_j1*delta_j1 + delta_j1*delta_j2*l + delta_j1*delta_j2*l +delta_j2*delta_j2*l*l + delta_j3*delta_j3*l * l
