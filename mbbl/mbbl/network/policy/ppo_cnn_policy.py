...
class policy_network(ppo_policy.policy_network):
    '''
        @brief:
    ...
    def _build_ph(self):
        ...
        self._input_ph = {
            'start_state': tf.placeholder(
                tf.float32, [None, self._image_height, self._image_width,
                             self._image_channel],
                name='start_state'
            )
        }
    ...
    def build_network(self):
        ...
        # the mlp for policy
        self._MLP = tf_networks.CNN_RL(
            output_size=self._action_size, scope='policy_mlp', cnn_type='a2c',
            train=True, activation_type=['relu'], normalizer_type=[None]
        )
        # the output policy of the network
        self._tensor['action_dist_mu'], self._tensor['pred_value'] = \
            self._MLP(self._tensor['net_input'])
